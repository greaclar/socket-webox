...
+ 可实现渐进式的心跳检测机制，区分网络抖动、网络差、客户端掉网。
+ 提供服务断线重连、实例化socket-webox的工厂函数，如果浏览器不兼容会返回null。使用更方便。
...
> websocket和http一样是应用层协议，在传输层使用了TCP。但地址以ws:或wss:开头。它与http的区别在于，websocket可以互相发起请求。当有新消息时，服务的主动通知客户端，无需客户端主动向服务器询问。
...
+ 可设置连续超时一定次数后，才派发心跳响应超时事件，防止因为网络抖动误报，用户体验更好。
  
+ 可在超时事件派发时，提醒用户网络差，然后设置更久的响应等待时间，并重新发起心跳检测。通过判断超时事件触发阈值，以提示用户，可能已经断网。
